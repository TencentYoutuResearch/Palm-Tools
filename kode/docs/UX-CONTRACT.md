...
| Capture screenshot | Global shortcut or Capture settings action | Duplicate capture blocked while the window or selected area is captured | Same application context | Image is placed directly on the system clipboard; success toast confirms the copy | Cancellation is silent; capture or clipboard failure uses an error toast | Existing application focus is preserved | screenshot platform contract |
