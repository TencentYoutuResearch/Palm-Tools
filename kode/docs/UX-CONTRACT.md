...
| Session switcher row | Quiet ledger surface without separate lifecycle text/dots; status remains in the row's accessible name, and idle avatars have no corner dot | Border and surface strengthen | Visible inset ring | One outer current-state treatment | n/a | Session status remains stable | Busy/attention/error stays local to the avatar or badge |
| Mobile session header | Backend artwork, title, exact working path, textual status, model, and usage in one compact AppBar; provider text and numeric IDs stay hidden | n/a | Long model/workspace values available by touch/focus tooltip | Permission mode remains a separate compact control | n/a | Geometry remains stable while metadata and status arrive | Missing metadata is omitted without placeholder noise |
| Mobile transcript message | One role bubble plus external identity marker | n/a | Selectable message content | Long-press text selection | n/a | Stable message geometry | Source text remains available; rendering fallback stays readable |
| Mobile agent activity | Hidden while idle/exited | n/a | Live text status and backend identity remain explicit | Appears at the transcript tail for starting/busy | n/a | Indeterminate indicator respects reduced motion | Disappears only from an authoritative status transition |
...
| Send session message | Send from session composer | Composer submit is blocked until dispatch response | Same session | Reflected semantic message/command status | Offline returns recoverable error and preserves message intent | Composer | cloud sync protocol §Mobile compatibility API |
...
- Mobile message routing is online-only and server-dispatched; the UI must not describe a `409 desktop offline` response as queued or delivered.
- Mobile working state is derived only from authoritative `session.status`: `busy` shows “Agent is working,” `starting` shows startup activity, and `idle`/`exited` removes the transcript activity line. Message timing or an unfinished animation must never invent presence.
