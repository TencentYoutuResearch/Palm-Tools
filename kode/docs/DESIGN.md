...
PTY foreground content preserves each child CLI's native ANSI choices, including CodeBuddy's truecolor green identity. `terminal_settings.ts` owns xterm's background, foreground, cursor, standard palette, and semantic surface slots for dark/light modes. The shared `terminal_ansi_theme.ts` adapter maps only explicit extended background colors—such as input fields and added/removed diff surfaces—into those slots, so existing cells recolor immediately when the theme changes; foreground groups are parsed atomically and never rewritten. Kode also passes `TERM_THEME` and `COLORFGBG` when a session starts so CLIs that support theme detection can choose their own palette, while xterm contrast correction keeps literal foreground colors readable.
...
Mobile session identity stays continuous from the session list into the detail header and assistant messages. Backend artwork carries the visible provider identity, so list rows and the detail header do not repeat a backend name or expose internal numeric session IDs. The compact detail AppBar pairs artwork and title with the exact working path plus live text status; a 28px instrument rail directly below retains model, token usage, and context pressure without a separate overview card. The grouped session list lets its folder header own the exact workspace path, while each compact row preserves title, live status, model, tokens, context, and attention state. Transcripts use one role surface per message, paired with a compact identity marker outside the bubble and a quiet timestamp in the message eyebrow. While the authoritative session status is `busy` or `starting`, a slim backend-owned activity line sits at the live end of the transcript and disappears when the session settles. Assistant identity comes from the actual session backend and reuses the desktop backend artwork with a readable monogram fallback; user and system roles use stable semantic glyphs. The composer uses a flat bordered input and a compact rounded-square upward-arrow commit button, not a floating circular paper-plane control. Transport protocol sentinels are rendered as plain technical labels rather than Markdown quote depth. Authored blockquotes remain flat, with a single slim rail and no nested rounded-card treatment.
