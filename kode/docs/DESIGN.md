...
PTY foreground content preserves each child CLI's native ANSI choices, including CodeBuddy's truecolor green identity. `terminal_settings.ts` owns xterm's background, foreground, cursor, standard palette, and semantic surface slots for dark/light modes. The shared `terminal_ansi_theme.ts` adapter normalizes standard, bright, low-index, 256-color, and truecolor red/green backgrounds to semantic removed/added slots: dark mode uses the muted `#3A2024`/`#173A27` surfaces and light mode uses the bright `#FF6B6B`/`#71D47D` surfaces. This keeps every diff internally consistent while foreground groups remain atomic and untouched. Kode also passes `TERM_THEME` and `COLORFGBG` when a session starts so CLIs that support theme detection can choose their own palette, while xterm contrast correction keeps literal foreground colors readable.
