...
impl Row {
    ...
    pub fn truncate(&mut self, len: u16) {
        ...
        if let Some(last_cell) = self.cells.last_mut() {
            // Truncating between the two cells of a wide glyph must also
            // clear the retained first half. Otherwise clear_wide() later
            // assumes a continuation cell exists at `col + 1` and panics.
