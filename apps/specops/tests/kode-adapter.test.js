...
describe('KodeClient', () => {
    ...
    test('creates a session and submits prompt via paste body then a separate CR', async () => {
        ...
        const server = createServer(async (request, response) => {
            ...
            if (request.url === '/api/v1/sessions') {
                response.writeHead(200, { 'content-type': 'application/json' })
                    .end(JSON.stringify({ id: 7, backend_key: 'codebuddy', status: 'starting' }));
            } else {
        ...
        const submitReq = requests[2]?.body;
